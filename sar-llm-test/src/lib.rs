...
impl Actor for LlmTestActor {
    ...
    async fn run(&self, bus: &SarBus) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let grammar_topic = "llm-test:0:grammar".to_string();
            ...
            tokio::select! {
                ...
                result = grammar_rx.recv() => {
                    match result {
                                    ...
                                    if grammar_type == "Grammar" {
                                        ...
                                        match content {
                                            Some(serde_json::Value::String(s)) => {
                                                info!(
                                                    "LLM test actor {} setting grammar, length={}",
                                                    self.index, s.len()
                                                );
                                            ...
                                            _ => {
                                                info!("LLM test actor {} clearing grammar", self.index);
                                                let mut g = self.grammar.lock().unwrap();
                                                *g = None;
                                            }
                                        }
                        ...
                        Err(RecvError::Lagged(n)) => {
                            warn!(
                                "LLM test actor {} lagged behind on grammar, dropped {} messages",
                                self.index, n
                            );
                        }
                        Err(RecvError::Closed) => {
                            info!("LLM test actor {} grammar topic closed", self.index);
                        }
                    }
                }
            }
