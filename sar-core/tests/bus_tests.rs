...
async fn test_bus_publish_and_subscribe() {
    ...
    let mut rx = bus.subscribe("test:pubsub").await.unwrap();
...
async fn test_bus_multiple_subscribers() {
    ...
    let mut rx1 = bus.subscribe("test:multi").await.unwrap();
    let mut rx2 = bus.subscribe("test:multi").await.unwrap();
...
async fn test_bus_clone_can_subscribe() {
    ...
    let bus2 = bus1.clone();
    let mut rx = bus2.subscribe("clone:test").await.unwrap();
    
    let msg = Message::text("clone:test", "sender", "cloned");
    bus1.publish(msg).await.unwrap();
...
async fn test_bus_lagged_receiver() {
    ...
    let mut rx = bus.subscribe("test:lag").await.unwrap();
...
async fn test_bus_message_format() {
    ...
    let mut rx = bus.subscribe("test:format").await.unwrap();
